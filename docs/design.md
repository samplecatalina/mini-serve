...
- **CUDA Graphs** are captured for decode only, one graph per batch-size bucket (powers of two up to the running-batch limit, and the limit itself), with padding up to the bucket; padding rows write and read a spare KV block that the allocator never hands out, so they cannot touch a request's KV. Token ids, positions and KV slots go through one pinned staging buffer into the graphs' static inputs; FlashInfer's CUDA-graph decode wrappers take the paged-KV metadata in buffers fixed at capture, which `plan` overwrites every step. Sampling stays outside the graph. Graphs are captured after the KV pool is allocated, largest first into one shared memory pool. Measured effect on the development GPU: an eager decode step at small batch is bound by kernel submission, about 19.5 ms whatever the batch; replayed from a graph it takes 8–10 ms. The saving is not only CPU time: queuing eager kernels ahead of time (so the CPU is never what the GPU waits for) still costs about 4 ms more GPU time per step than the graph, so on this platform every separately submitted kernel also costs a few microseconds of GPU time. When comparing against another engine, graph bucket configurations are aligned and recorded.
...
Status: M0, M1 and M2 complete (M2: prefix cache and chunked prefill, measured on the development GPU); in M3, CUDA Graph decode is done. Numbers will appear in `results/` and be discussed in `optimization-log.md` as they are measured.
