...
- **Chunked prefill batched with decodes.** With a per-step token budget (`--chunked-prefill-size`, default 2048; 0 restores prefill-only steps), each step first gives every decoding request its token, then spends the rest of the budget on prefill chunks: requests already part-way through their prefill first, then new admissions in arrival order. Admission still reserves blocks for the whole prefill plus one token; chunks allocate them step by step, and a request can be preempted between chunks (its computed full blocks stay in the prefix cache). A step with both decode rows and prefill chunks runs through the prefill attention kernel, where a decode row is a causal row of length one. Two measured consequences shape how the budget should be chosen. The tail of the inter-token latency depends on how many decode steps a prefill stall is spread over, not only on how long the longest stall is: with a few long prompts in a stream of short requests, 2048-token chunks cut the longest gap but double the 99th percentile, while 512-token chunks lower both. And chunking can raise throughput, because decode steps leave the GPU partly idle (CPU-side overhead per step), and a prefill chunk fills that time at a fraction of its cost in a prefill-only step.
...
Status: M0, M1 and M2 complete (M2: prefix cache and chunked prefill, measured on the development GPU). Numbers will appear in `results/` and be discussed in `optimization-log.md` as they are measured.
