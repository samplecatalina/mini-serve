...
Status: M0, M1 and M2 complete (M2: prefix cache and chunked prefill, measured on the development GPU); M3 complete (CUDA Graph decode, overlap scheduling, scheduling policies). Numbers will appear in `results/` and be discussed in `optimization-log.md` as they are measured.
