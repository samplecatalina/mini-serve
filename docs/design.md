...
Core types: `Request` (prompt ids, sampling params, state machine WAITING → PREFILL → DECODE → FINISHED/ABORTED), `Batch` (requests in this step plus block tables and positions), `BlockTable` (request → physical blocks).
...
- **KV pool size is a first-class experiment variable.** The pool is auto-sized at startup (profile peak activations after loading weights, give 90% of the remainder to KV, derive block count from the model config), but `--kv-pool-tokens N` overrides it. On a large GPU the pool is big enough that scheduling policies converge; sweeping the pool size turns that hardware limit into a controlled axis.
